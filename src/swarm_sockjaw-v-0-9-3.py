...
# 
# SOCJaw v0.9.3: this version has significant optimisations and general tidying up of code - mostly err fixes,
# but there's a lil placeholder filling, and also has robots.txt respecting code added so he don't go nowhere he not wnated :)
# note: we at 4011+ lines now - he a beeeg jellyfishings!
# 
...
def crawl_page(random_url, user_agent='user_agent'):
    ...
    def decide(self):
        """
        Decide whether to move on or continue to sense based on what's found 
        ...
        # If we detect certain keywords, we sense more, else move on
        ...
        if any(k in latest_data for k in keywords):
            ...
            print("[decide] No credential keywords found; move on.")
            self.crawl_page()
...
def parse_latest_data(latest_data):
    """
    A function that parses the text output 
    (e.g., from 'latest_data' script).
...
def test_parse():
    ...
    for i in range(10):
        self.decide()
        print(f"Cycle {i+1} => Position: {self.position}")

        # If any data was collected, let's show a short snippet
        if self.data:
            snippet = self.data[-1][:80].replace("\n", " ")
    ...
    # or parse the output from it:
        ...
        user_info, proxy_info, hw, net, plugins, history = parse_latest_data_output(...)
        # etc.

if __name__ == "__test_parse__":
    test_parse()
...
def main_password_generator():
    """
    Generator that yields possible brute-forced passwords.
    """
    common_passwords = [
    ...
    for pwd in itertools.chain(common_passwords):
        yield pwd
...
def main_password_check():
    # Assign the result of the brute_pass function to variables
    secret_password = main_password_check()
    net_auto_password = main_password_check()
    secret_password = 'password1'  # The first password we want to solve
    net_auto_password = 'password2' # The second password we want to solve
    print(f"Secret Password: {secret_password}")
    print(f"Net Auto Password: {net_auto_password}")
...
def text_str():
    ...
    for i, seq_pswd in enumerate(all_sequences):
        ...
        if is_rl_pswd(seq_pswd, secret_password):
            elapsed_time = time.time() - start_time  # Calculate elapsed time
            print(f"Correct password found: {seq_pswd}. Proceeding to login.")
            print(f"Elapsed time: {elapsed_time:.2f} seconds")
            return
    ...
    password_generator = main_password_generator()
    try:
        secret_password_gen = next(password_generator)
        net_auto_password_gen = next(password_generator)
        print(f"Secret Password (Gen): {secret_password_gen}")
        print(f"Net Auto Password (Gen): {net_auto_password_gen}")
        # Apply character mutation
        mutated_words = mutate(main_password_generator, char_map)
        ...
        for mutated_word in mutated_words:
            ...
            for case_variation in mutated_cases:
                ...
                for year_digit in year_digits:
                    ...
                    if is_rl_pswd(pswd_with_year_prepend, secret_password):
                        elapsed_time = time.time() - start_time  # Calculate elapsed time
        ...
        if is_rl_pswd(pswd_with_year_append, secret_password):
                        elapsed_time = time.time() - start_time  # Calculate elapsed time
    ...
    for i, combo in enumerate(combos):
        ...
        if is_rl_pswd(combo, net_auto_password):
            ...
            print(f"Elapsed time: {elapsed_time:.2f} seconds")
        if is_rl_pswd(combo, secret_password):
            elapsed_time = time.time() - start_time  # Calculate elapsed time
            print(f"Correct password found: {secret_password}. Pretending to accept the login.")
            print(f"Elapsed time: {elapsed_time:.2f} seconds")
            break
...
# Example of brute-force checking
    real_password1 = "password1"
    real_password2 = "password2"
    start_time = time.time()
    
    # Attempt to find the real password using the generator
    for pwd in main_password_generator():
        print(f"Trying password: {pwd}")
        if pwd == real_password1:
            elapsed = time.time() - start_time
            print(f"[Found] Password '{pwd}' matches the real password.")
            print(f"Elapsed Time: {elapsed:.2f} seconds")
        if pwd == real_password2:
            elapsed = time.time() - start_time
            print(f"[Found] Password '{pwd}' matches the real password.")
            print(f"Elapsed Time: {elapsed:.2f} seconds")
            break
    else:
        print("Password not found in the generator's list.")
...
def hash():
    ...
    hash_str = hash_password("secret_passwor")
    if compare_hashes(stored_hash, input_hash):
        print("Hash verification successful!")
    else:
        print("Hash verification failed.")
    hash_str = hash_password("net_auto_password")
    if compare_hashes(stored_hash, input_hash):
        print("Hash verification successful!")
    else:
        print("Hash verification failed.")
    ...
    for i, seq_hash in enumerate(all_seq_hashes):
        ...
        if is_rl_hash(seq_hash, secret_password):
            elapsed_time = time.time() - start_time # Calculate elapsed time
    ...
    for i, common_hash in enumerate(all_common_hashes):
        ...
        if is_rl_hash(common_hash, secret_password):
            elapsed_time = time.time() - start_time # Calculate elapsed time
